...
public class Book {
    ...
    static int currentIdNumber = 0;         //This will be unique for every book, since it will be incremented when every time
    //when a book is created


    public Book(int id,String t, String s, String a, boolean issued)    // Parametres cons.
    {
        currentIdNumber++;
        if(id==-1)
        {
    ...
    public void makeHoldRequest(Borrower borrower)
    {
        ...
        /**  If that borrower has already borrowed that particular book.
         Then he/she isn't allowed to make request for that book.
         He/she will have to renew the issued book in order to extend the return deadline.**/
         ...
         * Then he/she isn't allowed to make the same request again.**/
        ...
        else
        {
            if (!holdRequests.isEmpty())
            {
                ...
                if (hasRequest)
                {
                    ...
                    {
                        System.out.println("\n Book has been requested Sorry other users have requested for this book earlier than you. " +
                                "Please wait until their hold requests are processed.");
                        return;
                    }
                }
                else
                {
                    ...
                    System.out.println("Place the book on hold? (y/n)");
